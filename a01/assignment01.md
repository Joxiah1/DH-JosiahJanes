...
| **3. User Control and Freedom:**<br>The website effectively guides users to avoid most unwanted navigation or dialogue. | **Good:** The site makes great use of undo and redo features. <br>**Bad:** The site opens an additional tab each time the user clicks a link. While this is preferable to some users, it prevents the user from using the upper left arrows to navigate or backtrack to previous web pages. | Make links pop up in the same tab instead of opening additional tabs each time. | 2 |
| **4. Error Prevention:** The website effectively notifies the user of potential error submissions. | **Good:** The website uses error prevention messages to prevent users from blank submissions. | N/A | 1 |
